...
- В документации используй инфинитивы: "Получить сертификат по ИД.", а не "Получает сертификат по ИД."
...
- Для объединения свойств (данных) используй специальные прикладные структуры. Файлы располагаются в шаредной сборке как у модуля, так и у сущности.
- Примеры: \Packages\Sungero.Docflow\Sungero.Docflow.Shared\ModuleStructures.cs и \Packages\Sungero.Docflow\Sungero.Docflow.Shared\OfficialDocument\OfficialDocumentStructures.cs
- Структуры создавай через метод .Create() без параметров. Каждое свойство устанавливай отдельно.
...
- Запрещается генерировать и перехватывать родные исключения .NET. Например, исключение ArgumentOutOfRangeException
...
- LINQ запросы GetAll() репозитория, транслируются в SQL через NHibernate. С осторожностью используй конструкции .Contains(). Функция транслируется в SQL как in (:id1, :id2, id3..) кол-во id ограниченно для int, long, guid - 64000. Для string - 2100.
...
- Решение разделено на модули, примеры модулей - Commons, Company и др.
- Каждый модуль имеет виды кода: клиентский (Sungero.{Имя модуля}.ClientBase), разделяемый (Sungero.{Имя модуля}.Shared) и серверный (Sungero.{Имя модуля}.Server) код
- В модуле определяются различные типы сущностей. Каждый тип сущности определяется во всех видах кода. Например в модуле Sungero.Commons есть тип справочника Country, который есть и в клиентском и в разделяемом и в серверном коде
- В каждом виде кода есть функции и различные обработчики для типов сущностей. Например Sungero.{Имя модуля}.Server содержит тип сущности Country, у которой есть серверные обработчики CountryHandlers и серверные функции CountryServerFunctions
- В разделяемом коде есть структуры ({Название типа сущности или модуля}Structures) и константы ({Название типа сущности или модуля}Constants)
- У модуля и каждого типа сущности есть свои метаданные (расширение .mtd) и ресурсы (расширение .resx для английской локали и .ru.resx для русской локали). Метаданные и ресурсы лежат только в разделяемом коде.
- В клиентском коде можно вызывать функции из разделяемого и серверного кода
- В разделяемом коде можно вызывать функции из серверного кода
- В серверном коде можно вызывать функции из серверного и разделяемого кода
